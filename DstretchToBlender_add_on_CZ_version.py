bl_info = {
    "name": "DstertchToBlender Texture Switcher",
    "blender": (3, 0, 0),
    "category": "Object",
}
...
def switch_texture(self, context):
    ...
    if not obj or obj.type != 'MESH':
        self.report({'WARNING'}, "Vyberte prosím objekt typu Mesh.")
        return
    
    mat = obj.active_material
    if not mat or not mat.use_nodes:
        self.report({'WARNING'}, "Aktivní materiál nemá aktivované nody.")
        return
    ...
    if not image_textures:
        self.report({'WARNING'}, "Materiál neobsahuje žádné Image Texture nody.")
        return
    ...
    self.report({'INFO'}, f"Přepnuto na texturu: {image_texture_node.image.name}")

def modify_mtl_files():
    global folder_path
    
    if not folder_path:
        print("Nebyla zadána žádná cesta.")
        return
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.mtl'):
            ...
            for line in lines:
                ...
                if line.startswith('newmtl'):
                    if first_material_skipped:
                        ...
                        new_lines.append(f'map_Ke {rougness_texture if os.path.exists(os.path.join(folder_path, rougness_texture)) else rougness_texture2}\n')
            ...
            if second_material_found:
                ...
                print(f"Úspěšně upraveno: {filename}")
            else:
                print(f"Nebyly nalezeny žádné úpravy pro: {filename}")

class TEXTURE_OT_select_folder(bpy.types.Operator):
    bl_idname = "texture.select_folder"
    bl_label = "Vybrat složku"
    ...
    def execute(self, context):
        ...
        print(f"Vybraná složka: {folder_path}")
        return {'FINISHED'}
...
class TEXTURE_PT_switcher(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(context.scene, "folder_path", text="Cesta k .mtl")
        layout.operator("texture.change_mtl", text="Change .mtl")
        layout.operator("texture.select_folder", text="Vybrat složku")
...
def register():
    bpy.types.Scene.folder_path = bpy.props.StringProperty(
        name="Folder Path",
        description="Cesta k souborům .mtl",
        default=""
    )
